...
def process_excel_template(filename, metadata=None):
    ...
    for _, sheet in enumerate(wb):
        ...
        for i, row in enumerate(sheet):
            ...
            if "{%tr " in raw_values:
                ...
                style = [{"font": copy(c.font), "border": copy(c.border), "fill": copy(c.fill), "number_format": copy(c.number_format), "protection": copy(c.protection), "alignment": copy(c.alignment) } for c in row]
