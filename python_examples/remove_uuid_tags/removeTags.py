...
def retry_delete(key, val, attempts_remaining=5, interval=2):
    ...
    try:
        ...
        if attempts_remaining > 0 and delete_response.status_code == 504:
            time.sleep(interval)
            retry_request(attempts_remaining-1, interval*2)
...
"""
def retry_request(attempts_remaining=5, interval=2):
    try:
        # make request
    except Exception as e:
        if attempts_remaining > 0 and e.status_code == 504:
            time.sleep(interval)
            retry_request(attempts_remaining-1, interval*2)
        else:
            raise e
